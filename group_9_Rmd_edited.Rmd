...
films %>% group_by(genre) %>% summarise(count=n())
```
...
The $\chi^2$ value for the model can be calculated as 2330.2. The p-value corresponding to this $\chi^2$ value and 10 degrees of freedom is 0.000000. Since this p-value is much lower than 0.05, we would conclude that this model is highly useful.
...
table = matrix(c(fit$aic, fit2$aic), dimnames = list(c("fit","fit2"), c("AIC")))
table
```
The AIC value of the first model is less than that of the second model. Therefore, the first model is preferred.
...
It can be seen from the scatter plot matrix (figure \ref{fig:pairplot1}) that the scatter plots between length and year, votes and year, length and votes, budget and length, budget and votes all present a triangular shape, that is, there may be interactions, so these five interaction terms are put into the model for comparison
...
For further research, additional aspects such as film gross can be added.
